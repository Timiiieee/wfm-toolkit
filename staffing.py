...
def build_plan(rows, target_sl, target_seconds, interval_minutes):
    ...
    for label, calls, aht in rows:
        ...
        agents = erlang.required_agents(a, target_sl, target_seconds, aht)
...
def print_plan(plan, target_sl):
    header = (
        f"{'Interval':>8}  {'Calls':>5}  {'AHT':>4}  {'Load(E)':>7}  "
        f"{'Agents':>6}  {'SL':>6}  {'Occ':>5}  {'ASA(s)':>6}"
    )
    print(header)
    print("-" * len(header))
    ...
    for p in plan:
        ...
        print(
            ...
            f"{p['load_erlangs']:>7.2f}  {p['agents']:>6}  "
            f"{p['service_level']*100:>5.1f}%  {p['occupancy']*100:>4.0f}%  "
            f"{p['asa_seconds']:>6.1f}"
        )
    print("-" * len(header))
    print(f"Target service level: {target_sl*100:.0f}% within target time")
    if peak:
        print(f"Peak staffing: {peak['agents']} agents at {peak['interval']} "
              f"({peak['calls']} calls, {peak['load_erlangs']:.1f} Erlangs)")
    print(f"Sum of agent-intervals across the day: {total_agent_intervals}")
